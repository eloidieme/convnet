...
### TO-DO: room for improvement -> torch.kron and torch.cat ; find more efficient ways
...
class BalancedBatchSampler_Torch:
...
class CNN_Torch:
    ...
    def compute_loss(self, X_batch, Y_batch, F, W):
        ...
        _, _, P_batch = CNN_Torch.evaluate_classifier(X_batch, MFs, W)
    ...
    def compute_gradients(self, X_batch, Y_batch, F, W):
        ...
        X1_batch, X2_batch, P_batch = CNN_Torch.evaluate_classifier(X_batch, MFs, W)
        n = X_batch.shape[1]
        fact = 1/n
    ...
    def compute_accuracy(self, X, y, F, W):
        MFs = [CNN_Torch.make_mf_matrix(F[0], self.n_len),
               CNN_Torch.make_mf_matrix(F[1], self.n_len1)]
        P = CNN_Torch.evaluate_classifier(X, MFs, W)[-1]
    ...
    def mini_batch_gd(self):
        ...
        if self.balanced:
            balanced_batch_sampler = BalancedBatchSampler_Torch(self.y_train, n_samples_per_class, n_classes)
